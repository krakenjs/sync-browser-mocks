...
export function $mockEndpoint(options) {
  ...
  this.method =
    options.method instanceof RegExp
      ? options.method
      : // eslint-disable-next-line security/detect-non-literal-regexp
        new RegExp(options.method, "i");
  this.uri =
    // eslint-disable-next-line security/detect-non-literal-regexp
    options.uri instanceof RegExp ? options.uri : new RegExp(options.uri, "i");
...
$mockEndpoint.prototype = {
  ...
  onCall(listener) {
    ...
    const notCalled = this.expect && !this.called;
...
SyncXMLHttpRequest.prototype = {
  ...
  open(method, uri) {
    ...
    this.query = {};
    const params =
      uri.indexOf("?") >= 0 ? uri.slice(uri.indexOf("?") + 1).split("&") : [];
  ...
  getAllResponseHeaders() {
    return Object.keys(this.mock.headers)
      .map((key) => {
  ...
  _respond(status, response) {
    ...
    if (this._eventHandlers.load) {
      for (const handler of this._eventHandlers.load) {
        handler.call(this);
      }
    }

    const callback = this.onreadystatechange || this.onload;
